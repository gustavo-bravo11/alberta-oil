# Trans Mountain Throughput Input Contract
...
This contract defines the data-quality expectations for the CER Trans Mountain throughput and capacity CSV before it enters the pipeline.
...
| Dataset / report | Pipeline Throughput and Capacity Data - Trans Mountain Pipeline |
| File format | CSV |
| Delivery cadence | Monthly |
| Source location | https://open.canada.ca/data/en/dataset/dc343c43-a592-4a27-8ee7-c77df56afb34/resource/404fa9e0-73b1-4a8c-9f43-8045d7edb426 |
| Grain | One record per month, key point, and product; a `system` capacity row is permitted. |
...
| `Company` | string | Non-empty; must equal `Trans Mountain Pipeline ULC` | `Trans Mountain Pipeline ULC` |
| `Pipeline` | string | Non-empty; must equal `Trans Mountain pipeline` after trimming | `Trans Mountain pipeline` |
| `Key Point` | string | Non-empty; must be one of the permitted key points | `Burnaby`; `Sumas`; `Westridge`; `system` |
| `Latitude` | decimal | Required on product-flow rows and within 0 to 90; no rule on helper rows | `49.2` |
| `Longitude` | decimal | Required on product-flow rows and within -180 to 0; no rule on helper rows | `-123.1` |
| `Direction Of Flow` | string | Required on product-flow rows and must be a cardinal direction; no rule on helper rows | `north`; `south`; `east`; `west` |
| `Trade Type` | string | Required on product-flow rows and must be in the shared allowed domain; no rule on helper rows | `import`; `export`; `intracanada`; `intracanada / export` |
| `Product` | string | Must be one of the closed allowed values; blank is permitted only on a `system` helper row | `domestic heavy`; `domestic light`; `domestic light / ngl`; `foreign light`; `refined petroleum products`; `total`; blank on `system` helper rows only |
| `Throughput (1000 m3/d)` | decimal | Required, finite, and non-negative on product-flow rows; no rule on helper rows | `48.75` |
| `Committed Volumes (1000 m3/d)` | decimal | Optional on product-flow rows; no rule on helper rows | `62.8` |
| `Uncommitted Volumes (1000 m3/d)` | decimal | Optional on product-flow rows; no rule on helper rows | `2.5` |
...
| `Available Capacity (1000 m3/d)` | decimal | Optional on product-flow rows; when present, finite and positive. No rule on helper rows | `118.6` |
| `Reason For Variance` | string | Optional; when present, non-empty after trimming | `Maintenance` |

### Row types

- A **product-flow row** has `Key Point` equal to `Burnaby`, `Sumas`, or `Westridge`, and a product other than `total`. Its location, direction, trade type, and throughput fields must be populated.
- A **system capacity helper row** has `Key Point` equal to `system`. No field-level validation is applied to helper rows at this time.
...
| Date | Change | Author |
| --- | --- | --- |
| 2026-09-14 | Initial draft, aligned to the current Trans Mountain raw CSV. | Gustavo Bravo |
